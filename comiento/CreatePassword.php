...
if (email_exists($email,$conn)) {
  $result=mysqli_query($conn,"SELECT password FROM customer_login WHERE email='$email'");
  $retrive=mysqli_fetch_array($result);
  $password=$retrive['password'];
  if ($pass==$password) {
      
        $pwd=md5($pass);
        mysqli_query($conn,"UPDATE customer_login SET password='$pwd'");
...
<body class="bg-dark">
	...
	<form class="form-group" method="post" enctype="multipart/form data" action="Createpassword.php?email=<?php echo $_GET["email"];?>">	
